...
from wildgs_slam.depth_video import DepthVideo
from wildgs_slam.gui import gui_utils
from wildgs_slam.utils.camera_utils import Camera
from wildgs_slam.utils.common import as_intrinsics_matrix, setup_seed
from wildgs_slam.utils.datasets import get_dataset, load_img_feature, load_metric_depth
from wildgs_slam.utils.dyn_uncertainty import mapping_utils as map_utils
from wildgs_slam.utils.dyn_uncertainty.median_filter import MedianPool2d
from wildgs_slam.utils.plot_utils import create_gif_from_directory
from wildgs_slam.utils.pose_utils import update_pose
from wildgs_slam.utils.printer import FontColor, Printer
from wildgs_slam.utils.slam_utils import (
    get_loss_mapping,
...
class Mapper(object):
    ...
    def run(self):
        ...
        while True:
            ...
            if invalid:
                ...
                self.printer.print(
                    "WARNING: Too few valid pixels from droid depth", FontColor.MAPPER
                )
    ...
    def _get_viewpoint(self, video_idx: int, frame_idx: int) -> Tuple[Camera, bool]:
        ...
        if self.uncertainty_aware:
            features = load_img_feature(frame_idx, self.save_dir, suffix=load_feature_suffix).to(
                self.device
            )
        else:
            features = None
        ...
        est_depth, est_w2c, invalid = self.get_w2c_and_depth(video_idx, frame_idx, metric_depth)
    ...
    def _update_mapping_points(self, frame_idx, w2c, w2c_old, depth, depth_old, method=None):
        """Refer to splat-slam"""
        if method == "rigid":
            ...
            self.gaussians._xyz = self.gaussians.replace_tensor_to_optimizer(global_means, "xyz")[
                "xyz"
            ]
            ...
            self.gaussians._rotation = self.gaussians.replace_tensor_to_optimizer(rots, "rotation")[
                "rotation"
            ]
            ...
            self.gaussians._scaling = self.gaussians.replace_tensor_to_optimizer(scales, "scaling")[
                "scaling"
            ]
    ...
    def _update_occ_aware_visibility(self, current_window):
        ...
        for kf_idx in current_window:
            ...
            self.occ_aware_visibility[kf_idx] = (render_pkg["n_touched"] > 0).long()
        ...
        for mapping_iteration in range(self.init_itr_num):
            ...
            if not self.uncertainty_aware:
                ...
                loss_init += self.config["mapping"]["uncertainty_params"][
                    "reg_mult"
                ] * map_utils.compute_dino_regularization_loss(uncer_buffer, feature_buffer)
    ...
    def map_opt_online(self, current_window, iters=1):
        ...
        prob = np.full(
            len(viewpoint_stack),
            (1 - cur_window_prob) * iters / (len(viewpoint_stack) - len(current_window)),
        )
        ...
        for _ in tqdm(range(iters)):
            ...
            if not self.uncertainty_aware:
                ...
                (
                    ...
                    train_frac,
                    ssim_frac,
                    freeze_uncertainty_loss=self.iterations_after_densify_or_reset < 200,
                )
                loss_mapping += loss_mapping_this_frame

                stride = self.config["mapping"]["uncertainty_params"]["reg_stride"]
                uncer_buffer.append(current_uncertainty[::stride, ::stride].unsqueeze(-1))
    ...
    def _get_uncertainty_ssim_loss_vis(
        ...
        l, c, s = map_utils.compute_ssim_components(
            gt_image,
            rendered_img,
            window_size=self.config["mapping"]["uncertainty_params"]["ssim_window_size"],
        )
        ...
        median_filter = MedianPool2d(
            kernel_size=self.config["mapping"]["uncertainty_params"]["ssim_median_filter_size"],
            stride=1,
            padding=0,
            same=True,
        )
        ssim_loss = median_filter(ssim_loss.unsqueeze(0).unsqueeze(0)).squeeze(0).squeeze(0)
        opacity_th = self.config["mapping"]["uncertainty_params"]["opacity_th_for_uncer_loss"]
    ...
    def get_viewpoint_uncertainty_no_grad(self, viewpoint: Camera) -> torch.Tensor:
        ...
        uncertainty_resized = map_utils.resample_tensor_to_shape(uncertainty, target_shape)
    ...
    def save_fig_everything(
        self, keyframe_idx: int, plot_dir: str, suffix: str = "", depth_max: float = 10.0
    ):
        """
        ...
        mask = gt_image > 0
        psnr_score = psnr((rendered_img[mask]).unsqueeze(0), (gt_image[mask]).unsqueeze(0))
        ...
        if self.uncertainty_aware:
            ...
            ssim_loss = self._get_uncertainty_ssim_loss_vis(gt_image, rendered_img, opacity)
        ...
        save_path = os.path.join(
            plot_dir, f"video_idx_{keyframe_idx}_kf_idx_{frame_idx}{suffix}.png"
        )
    ...
    def _vis_uncertainty_mask_all(self, n_rows=8, n_cols=8, is_final=False):
        ...
        assert n_rows % 2 == 0  # one row for uncertainty, one for imgs, the other for uncertainty
        ...
        if n_img >= len(self.cameras):
            ...
            keyframe_idxs = (
                list(self.cameras.keys())[:n_cols] + list(self.cameras.keys())[-(n_img - n_cols) :]
            )
        ...
        for i in range(0, n_rows // 2):
            for j in range(n_cols):
                ...
                if idx >= len(keyframe_idxs):
                    ...
                    axs[2 * i + 1, j].imshow(uncer_resized**2, cmap="jet", vmin=0, vmax=5)
        ...
        if is_final:
            save_path = os.path.join(self.save_dir, "online_uncer", "after_final_refine.png")
        else:
