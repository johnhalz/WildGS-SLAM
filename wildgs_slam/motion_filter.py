...
from wildgs_slam.modules.droid_net import CorrBlock
from wildgs_slam.utils.mono_priors.img_feature_extractors import (
    get_feature_extractor,
    predict_img_features,
)
from wildgs_slam.utils.mono_priors.metric_depth_estimators import (
...
class MotionFilter:
    ...
    def track(self, tstamp, image, intrinsics=None):
        ...
        if self.video.counter.value == 0:
            ...
            mono_depth = predict_metric_depth(
                self.metric_depth_estimator, tstamp, image, self.cfg, self.device
            )
            if self.uncertainty_aware:
                dino_features = predict_img_features(
                    self.feat_extractor, tstamp, image, self.cfg, self.device
                )
            else:
                dino_features = None
        ...
        ### only add new frame if there is enough motion ###
            ...
            if self.cfg["tracking"]["force_keyframe_every_n_frames"] > 0:
                ...
                force_to_add_keyframe = (tstamp - last_tstamp) >= self.cfg["tracking"][
                    "force_keyframe_every_n_frames"
                ]
    ...
    def get_img_feature(self, tstamp, image, suffix=""):
        dino_features = predict_img_features(
            self.feat_extractor, tstamp, image, self.cfg, self.device, suffix=suffix
        )
