...
import wildgs_slam.geom.projective_ops as pops
from wildgs_slam.modules.droid_net import cvx_upsample
from wildgs_slam.utils.common import align_scale_and_shift
from wildgs_slam.utils.dyn_uncertainty import mapping_utils as map_utils
from wildgs_slam.utils.printer import FontColor
...
class DepthVideo:
    ...
    def __init__(self, cfg, printer, uncer_network=None):
        ...
        if not self.metric_depth_reg:
            self.printer.print("Metric depth for regularization is not activated.", FontColor.INFO)
            self.printer.print(
                "This should not happen for WildGS-SLAM unless you are doing ablation study",
                FontColor.INFO,
            )
        ...
        self.down_scale = 8
        self.slice_h = slice(
            self.down_scale // 2 - 1, ht // self.down_scale * self.down_scale + 1, self.down_scale
        )
        self.slice_w = slice(
            self.down_scale // 2 - 1, wd // self.down_scale * self.down_scale + 1, self.down_scale
        )
        ### state attributes ###
        ...
        self.disps_up = torch.zeros(
            buffer, ht, wd, device=self.device, dtype=torch.float
        ).share_memory_()
        self.intrinsics = torch.zeros(
            buffer, 4, device=self.device, dtype=torch.float
        ).share_memory_()
        ...
        self.mono_disps_up = torch.zeros(
            buffer, ht, wd, device=self.device, dtype=torch.float
        ).share_memory_()
        self.mono_disps_mask_up = torch.ones(
            buffer, ht, wd, device=self.device, dtype=torch.bool
        ).share_memory_()
        self.depth_scale = torch.zeros(
            buffer, device=self.device, dtype=torch.float
        ).share_memory_()
        self.depth_shift = torch.zeros(
            buffer, device=self.device, dtype=torch.float
        ).share_memory_()
        self.valid_depth_mask = torch.zeros(
            buffer, ht, wd, device=self.device, dtype=torch.bool
        ).share_memory_()
        ...
        if self.uncertainty_aware:
            ...
            if self.cfg["mono_prior"]["feature_extractor"] not in [
                ...
                raise ValueError(
                    "You are using a new feature extractor, make sure the downsample factor is 14"
                )

            # The followings are in cpu to save memory
            self.dino_feats = torch.zeros(
                buffer, ht // 14, wd // 14, n_features, device="cpu", dtype=torch.float
            ).share_memory_()
            ...
            self.uncertainties_inv = torch.ones(
                buffer,
                ht // self.down_scale,
                wd // self.down_scale,
                device=self.device,
    ...
    def __item_setter(self, index, item):
        ...
        if len(item) > 9 and item[9] is not None:
            ...
            if len(item[9].shape) == 3:
                self.dino_feats_resize[index] = (
                    F.interpolate(
                        item[9].permute(2, 0, 1).unsqueeze(0),
                        self.disps_up.shape[-2:],
                        mode="bilinear",
                    )
                    .squeeze()[:, self.slice_h, self.slice_w]
                    .cpu()
                )
            else:
                self.dino_feats_resize[index] = F.interpolate(
                    item[9].permute(0, 3, 1, 2), self.disps_up.shape[-2:], mode="bilinear"
                )[:, :, self.slice_h, self.slice_w].cpu()
    ...
    def filter_high_err_mono_depth(self, idx, ii, jj):
        ...
        X0, _ = pops.iproj(
            self.mono_disps_up[jj].unsqueeze(0),
        ...
        valid_mask = (
            (x1_rounded[..., 1] >= 0)
            & (x1_rounded[..., 1] < x1.shape[2])
            & (x1_rounded[..., 0] >= 0)
            & (x1_rounded[..., 0] < x1.shape[3])
            & (x1[..., 2] > 0)
        )

        i_dino = (
            F.interpolate(
                self.dino_feats[idx].permute(2, 0, 1).unsqueeze(0),
                self.disps_up.shape[-2:],
                mode="bilinear",
            )
            .to(self.device)
        ...
        for j_id in range(jj.shape[0]):
            ...
            j_dino = (
                F.interpolate(
                    self.dino_feats[jj[j_id]].permute(2, 0, 1).unsqueeze(0),
                    self.disps_up.shape[-2:],
                    mode="bilinear",
                )
                .to(self.device)
            ...
            similarity = (
                F.normalize(j_dino_valid, p=2, dim=0)
                .mul(F.normalize(i_dino_valid, p=2, dim=0))
                .sum(dim=0)
            )
            ...
            error = (
                torch.abs(
                    1 / j_projected_disp[matched_y, matched_x] - 1 / i_disp[matched_y, matched_x]
                )
                * j_projected_disp[matched_y, matched_x]
            )
        ...
        self.mono_disps_mask_up[idx][
            (accurate_count <= 1) & (inaccurate_count > 0) & (self.mono_disps_up[idx] > 0)
        ] = False
    ...
    def ba(
        ...
        with self.get_lock():
            ...
            if not self.metric_depth_reg:
                ...
                mono_valid_mask = (
                    self.mono_disps_mask_up[:, self.slice_h, self.slice_w].clone().to(self.device)
                )
                ...
                droid_backends.ba(
                    ...
                    self.disps,
                    self.intrinsics[0],
                    self.mono_disps * mono_valid_mask,
                    target,
                    weight,
                    eta,
                    ii,
    ...
    def get_depth_scale_and_shift(
        self, index, mono_depth: torch.Tensor, est_depth: torch.Tensor, weights: torch.Tensor
    ):
        """
        index: int
        mono_depth: [B,H,W]
    ...
    def update_all_uncertainty_mask(self):
        if not self.uncertainty_aware:
            ...
            raise Exception(
                "This function should not be called if uncertainty aware is not activated"
            )

        i = 0
        while i * 20 < self.counter.value:
            dino_feat_batch = self.dino_feats[
                i * 20 : min((i + 1) * 20, self.counter.value), :, :, :
            ].to(self.device)
            with Lock():
            ...
            self.uncertainties_inv[i * 20 : min((i + 1) * 20, self.counter.value), :, :] = (
                torch.clamp(0.5 / uncer**2, 0.0, 1.0)
            )
    ...
    def update_uncertainty_mask_given_index(self, idxs):
        ...
        uncer = uncer.unsqueeze(1)
        uncer = (
            torch.nn.functional.interpolate(uncer, size=(h, w), mode="bilinear").squeeze(1).detach()
        )
        data_rate = 1 + 1 * map_utils.compute_bias_factor(train_frac, 0.8)
        uncer = uncer[:, self.slice_h, self.slice_w]
        uncer = (uncer - 0.1) * data_rate + 0.1
    ...
    def eval_depth_l1(self, npz_path, stream, global_scale=None):
        ...
        for i in range(video_timestamps.shape[0]):
            ...
            if global_scale is None:
                scale, shift, _ = align_scale_and_shift(
                    depth.unsqueeze(0), depth_gt.unsqueeze(0), mask.unsqueeze(0)
                )
