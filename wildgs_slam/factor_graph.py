...
from wildgs_slam.modules.droid_net import AltCorrBlock, CorrBlock
...
class FactorGraph:
    ...
    @torch.no_grad()
    def update_lowmem(
        self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, steps=8, enable_wq=True
    ):
    ...
    def add_backend_proximity_factors(
        self, t_start, t_end, nms, radius, thresh, max_factors, beta, t_start_loop=None, loop=False
    ):
        ...
        for k in ix:
            ...
            if loop:
                ...
                num_loop = 0
                for si in range(
                    max(i - n_neighboring, t_start_loop), min(i + n_neighboring + 1, t_end)
                ):
                    for sj in range(
                        max(j - n_neighboring, t_start), min(j + n_neighboring + 1, t_end)
                    ):
