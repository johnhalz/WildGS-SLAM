...
from wildgs_slam.backend import Backend as LoopClosing
from wildgs_slam.factor_graph import FactorGraph
...
class Frontend:
    ...
    def __update(self, force_to_add_keyframe):
        ...
        for itr in range(self.iters1):
            ...
            if not self.cfg["fast_mode"]:
                if (
                    itr == 1
                    and self.video.metric_depth_reg
                    and self.cfg["tracking"]["uncertainty_params"]["activate"]
                ):
        ...
        if (
            (d.item() < self.keyframe_thresh)
            & (self.num_keyframes_dropped < self.max_consecutive_drop_of_keyframes)
            & (not force_to_add_keyframe)
        ):
    ...
    def initialize_second_stage(self):
        ...
        self.graph.age = torch.maximum(
            self.graph.age - 8, torch.tensor(0).to(self.graph.age.device)
        )
