...
from wildgs_slam.gui.gl_render import util, util_gau
from wildgs_slam.gui.gl_render.render_ogl import OpenGLRenderer
from wildgs_slam.gui.gui_utils import (
    GaussianPacket,
    Packet_vis2main,
    create_frustum,
    cv_gl,
...
class SLAM_GUI:
    ...
    def receive_data(self, q):
        ...
        if gaussian_packet.current_frame is not None:
            ...
            if self.followcam_chbox.checked:
                viewpoint = (
                    frustum.view_dir_behind if self.staybehind_chbox.checked else frustum.view_dir
                )
                self.widget3d.look_at(viewpoint[0], viewpoint[1], viewpoint[2])
        ...
        if gaussian_packet.gtdepth is not None:
            ...
            depth = imgviz.depth2rgb(depth, min_value=0, max_value=5.0, colormap="jet")
        ...
        if gaussian_packet.uncertainty is not None:
            ...
            uncertainty = imgviz.depth2rgb(uncertainty, min_value=0, max_value=5.0, colormap="jet")
    ...
    def depth_to_normal(points, k=3, d_min=1e-3, d_max=10.0):
        ...
        vec_vert = (
            points_pad[:, :, :h, k : w + k] - points_pad[:, :, 2 * k : h + (2 * k), k : w + k]
        )

        # horizontal vector (left - right)
        vec_hori = (
            points_pad[:, :, k : h + k, :w] - points_pad[:, :, k : h + k, 2 * k : w + (2 * k)]
        )
    ...
    def rasterise(self, current_cam):
        if (
            ...
            alpha = 0.1
            self.gaussian_cur.get_features = alpha * features + (1 - alpha) * torch.from_numpy(
                rgb_kf
            ).to(features.device)
