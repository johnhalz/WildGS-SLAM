...
class Backend:
    ...
    def backend_ba(
        ...
        edge_num = graph.add_backend_proximity_factors(
            t_start, t_end, nms, radius, thresh, max_factors, self.beta, t_start_loop, loop
        )
        if edge_num == 0:
            graph.clear_edges()
            return 0
    ...
    def dense_ba(self, steps=6, enable_wq=True):
        ...
        graph = FactorGraph(
            self.video, self.update_op, device=self.device, corr_impl="alt", max_factors=max_factors
        )
        n_edges = self.backend_ba(
            t_start,
            t_end,
            steps,
            graph,
