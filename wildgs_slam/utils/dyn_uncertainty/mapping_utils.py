...
def resample_tensor_to_shape(
    ...
    return F.interpolate(tensor, size=target_shape, mode=interpolation_mode).squeeze(0).squeeze(0)
...
def _ssim(
    ...
    sigma1_sq = (
        F.conv2d(img1 * img1, window, padding=window_size // 2, groups=num_channels) - mu1_sq
    )
    sigma2_sq = (
        F.conv2d(img2 * img2, window, padding=window_size // 2, groups=num_channels) - mu2_sq
    )
    ...
    structure = (sigma12 + SSIM_C3) / (torch.sqrt(sigma1_sq) * torch.sqrt(sigma2_sq) + SSIM_C3)
...
def compute_mapping_loss_components(
    ...
    uncertainty_loss = (
        ...
        + uncertainty_config["uncer_depth_mult"] * small_depth_loss / processed_uncertainty**2
    )
    uncertainty_loss[small_opacity < uncertainty_config["opacity_th_for_uncer_loss"]] = 0
...
def compute_dino_regularization_loss(
    ...
    uncertainty_variances = torch.sum(squared_differences, dim=1) / valid_neighbor_counts
