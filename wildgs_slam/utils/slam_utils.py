...
def get_loss_tracking(config, image, depth, opacity, viewpoint, monocular=True, uncertainty=None):
    ...
    if monocular:
        ...
        raise NotImplementedError(
            "Only implemented monocular, not rgbd for uncertainty-aware tracking"
        )
...
def get_loss_tracking_rgbd(config, image, depth, opacity, viewpoint, uncertainty=None):
...
def get_loss_mapping_uncertainty(
    ...
    viewpoint,  # from wildgs_slam.utils.camera_utils import Camera, to avoid loop import
    opacity: Tensor,
    ...
    rendered_img = (
        rendered_img
        if initialization
        else (torch.exp(viewpoint.exposure_a) * rendered_img + viewpoint.exposure_b)
    )
    ...
    if config["full_resolution"]:
        weights = F.interpolate(weights.unsqueeze(0), l1_depth.shape[-2:], mode="bilinear").squeeze(
            0
        )
