...
from wildgs_slam.utils.datasets import RGB_NoPose
...
def full_traj_eval(
    traj_filler, mapper, plot_parent_dir, plot_name, stream, logger, printer, fast_mode=False
):
    ...
    if not fast_mode:
        ...
        for i in tqdm(range(traj_est.shape[0])):
            ...
            w2c_refined = mapper.refine_pose_non_key_frame(
                i, torch.tensor(np.linalg.inv(traj_est[i])), features=img_feat
            )
