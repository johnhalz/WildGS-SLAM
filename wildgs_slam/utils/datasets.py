...
class BaseDataset(Dataset):
    def __init__(self, cfg, device="cuda:0"):
        ...
        self.H_out_full, self.W_out_full = (
            self.H - self.H_edge_full * 2,
            self.W - self.W_edge_full * 2,
        )
        ...
        if "ROOT_FOLDER_PLACEHOLDER" in self.input_folder:
            self.input_folder = self.input_folder.replace(
    ...
    def get_color_full_resol(self, index):
        ...
        color_data_fullsize = (
            torch.from_numpy(color_data_fullsize).float().permute(2, 0, 1)[[2, 1, 0], :, :] / 255.0
        )  # bgr -> rgb, [0, 1]
    ...
    def __getitem__(self, index):
        ...
        if depth_data_fullsize is not None:
            ...
            depth_data = F.interpolate(depth_data_fullsize[None, None], outsize, mode="nearest")[
                0, 0
            ]
        else:
