...
class Fit3DModels(torch.nn.Module):
    ...
    def get_intermediate_layers(
        ...
        if return_class_token:
            ...
            prefix_tokens = [out[:, 0 : self.model.num_prefix_tokens] for out in outputs]
        outputs = [out[:, self.model.num_prefix_tokens :] for out in outputs]

        if reshape:
            B, C, H, W = x.shape
            grid_size = (
                (H - self.model.patch_embed.patch_size[0]) // self.model.patch_embed.proj.stride[0]
                + 1,
                (W - self.model.patch_embed.patch_size[1]) // self.model.patch_embed.proj.stride[1]
                + 1,
            )
            outputs = [
